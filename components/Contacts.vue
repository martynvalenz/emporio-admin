			...
			<v-container fluid grid-list-xl>
				<v-layout wrap class="pt-2">
					<v-flex xs12 sm12 md4 lg4 xl4>
						<v-btn color="primary" dark @click="createContact"><v-icon left>person_add</v-icon>Agregar contacto</v-btn>
					</v-flex>
					<v-flex xs12 sm12 md4 lg4 xl4>
						<v-select label="Agregar contactos existentes" outlined color="primary"></v-select>
					</v-flex>
					<v-flex xs12 sm12 md4 lg4 xl4 class="text-right">
						<v-btn icon class="ma-2 white--text" @click="LoadContacts"><v-icon>sync</v-icon></v-btn>
					</v-flex>
				</v-layout>
				<v-layout wrap>
					<v-flex xs12>
							...
							<v-simple-table class="elevation-1">
								...
								<tbody>
									<tr v-for="(contact, index) in contacts" :key="index">
										<td><span v-if="contact.title">{{ contact.title }} </span>{{ contact.name }}</td>
										<td>{{ contact.position }}</td>
										<td>{{ contact.email }}</td>
										<td>{{contact.phone}}<span v-if="contact.extension"> {{ contact.extension }}</span></td>
										<td>
...
export default {
	...
	methods:{
		...
		async LoadContacts(){
			...
			await this.$axios.get(`/api/customer/contacts/${this.customer}`)
			.then(res =>{
