		...
		<v-layout class="pt-4">
			<v-flex xs12>
				<v-card :loading="bills_loading">
					<v-card-title>
						<v-btn color="secondary" class="mx-1" @click="addBill">Recibo<v-icon right>add</v-icon></v-btn>
...
export default {
	...
	methods:{
		...
		async Load(){
			...
			await this.$axios.post(this.url, {search:this.search_table, orderBy:this.orderBy, type:'Recibo'})
			.then(res => {
		...
		infiniteScroll($state){
			setTimeout(() => {
				this.page++
				this.$axios.post(this.url, {search:this.search_table, orderBy:this.orderBy, type:'Recibo'})
				.then( res => {
		...
		editBill(index){
			...
			this.$refs.bills_form.editBill(this.bills[index].id,3);
		},
