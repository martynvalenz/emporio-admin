...
export default {
	...
	computed: {
		url(){
			return `${process.env.api}/api/bills/pending?page=${this.page}`
		}
	},
	...
	methods:{
		...
		async Load(){
			...
			await this.$axios.post(this.url, {search:this.search_table, orderBy:this.orderBy})
			.then(res => {
		...
		infiniteScroll($state){
			setTimeout(() => {
				this.page++
				this.$axios.post(this.url, {search:this.search_table, orderBy:this.orderBy})
				.then( res => {
