						...
						<v-simple-table class="elevation-3" fixed-header height="700px">
							<thead>
								<tr>
									...
									<th class="text-center" style="width:22%">Trámite/Vencimiento</th>
							...
							<tbody>
								<tr v-for="(service, index) in services" :key="index">
									...
									<td class="text-center">
										<v-chip label small v-if="service.status == 1 && service.expiration" class="orange" dark>{{ service.expiration }}</v-chip>
										<v-chip label small v-if="service.status == 1 && !service.expiration" class="orange" dark>Pendiente</v-chip>
...
export default {
	...
	methods:{
		...
		async Load(){
			...
			await this.$axios.post(this.url, {search:this.search_table, status:this.status, orderBy:this.orderBy, binnacle:5})
			.then(res => {
		...
		infiniteScroll($state){
			setTimeout(() => {
				this.page++
				this.$axios.post(this.url, {search:this.search_table, status:this.status, orderBy:this.orderBy, binnacle:5})
				.then( res => {
